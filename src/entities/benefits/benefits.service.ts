...
export class BenefitsService {
  constructor(
    ...
    this.logger.setContext(BenefitsService.name);
  }
  ...
  async get_categories(
    id_partner: string,
  ): Promise<PartnersCategoriesReturn | null> {
    if (!id_partner) throw new BadRequestException('ID partner is required');
    const fullPartner =
      await this.partnersService.get_by_id_with_categories(id_partner);
  ...
  async update(dto: BenefitsUpdateDTO): Promise<BenefitsReturn> {
    ...
    if (dto.max_coupons !== undefined) benefit.max_coupons = dto.max_coupons;
    if (dto.max_per_user !== undefined) benefit.max_per_user = dto.max_per_user;
    if (dto.status !== undefined) benefit.status = dto.status as BenefitStatus;

    await this.benefitsRepository.save(benefit);
    return await this.get_benefit(benefit.id_benefit);
  }

  async incrementCoupons(
    id_benefit: string,
    maxCoupons: number,
  ): Promise<boolean> {
  ...
  private async mapBenefit(benefit: BenefitsEntity): Promise<BenefitsReturn> {
    if (!benefit) throw new BadRequestException('Invalid benefit');
    const paymentMethodsNames: string[] = (benefit.payment_methods ?? []).map(
      (p) => p.name,
    );
  ...
  private async mapBenefits(
    benefits: BenefitsEntity[],
  ): Promise<BenefitsReturn[]> {
