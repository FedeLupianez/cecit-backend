...
export class PartnersCategoriesService {
    ...
    async create(data: PartnersCategoriesDto, callerId: string) {
        ...
        const category = await this.categoriesRepo.findOneBy({ id_category: data.id_category });
        if (!category) throw new BadRequestException('Category not found');
        if (partner.categories.some((c) => c.id_category === category.id_category)) return partner;
        partner.categories.push(category);
        return await this.partnersRepo.save(partner);
    }

    async findAll() {
        const partners = await this.partnersRepo.find({ relations: ['categories'] });
        // Return flattened for backward compat
        return partners.flatMap((p) =>
            p.categories.map((c) => ({ id_partner: p.id_partner, id_category: c.id_category, partner: p, category: c })),
        );
    }
