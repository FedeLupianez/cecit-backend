...
export class VouchersService {
  ...
  async mapVoucher(voucher: VouchersEntity): Promise<VoucherReturn> {
    const relatedBenefit = await this.benefitsService.get_benefit(
      voucher.id_benefit,
    );
    if (!relatedBenefit) throw new BadRequestException('Invalid Voucher');
    return {
  ...
  async get_by_token(
    ...
    return {
      ...
      directions: (voucher.benefit.partner.directions ?? []).map(
        (d) => d.direction,
      ),
  ...
  async get_by_user_benefit(
    vouchers: VoucherBenefitUser,
  ): Promise<ReturnCouponsUser> {
    ...
    this.logger.debug(
      `Result of ${vouchers.id_benefit} | ${vouchers.id_account} = ${total}`,
    );
    return {
      id_account: vouchers.id_account,
      coupons: total,
  ...
  async create(voucher: VouchersCreateDTO) {
    ...
    newVoucher.token = await generateUniqueToken(
