...
export class PaymentBenefitService {
    ...
    async make_relation(id_benefit: string, payment_method: string): Promise<boolean> {
        const method = await this.paymentMethodsRepo.findOneBy({ name: payment_method });
        ...
        if (benefit.payment_methods.some((pm) => pm.id_payment_method === method.id_payment_method)) return true;
