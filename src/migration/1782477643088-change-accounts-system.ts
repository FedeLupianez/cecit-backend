...
export class ChangeAccountsSystem1782477643088 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`RefreshTokens\` (\`id_token\` uuid NOT NULL, \`token_hash\` varchar(255) NOT NULL, \`expires_at\` datetime NOT NULL, \`revoked\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`IDX_a8902d4754e89411bbc04a9774\` (\`token_hash\`), PRIMARY KEY (\`id_token\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`Vouchers\` (\`token\` varchar(6) NOT NULL, \`id_user\` varchar(4) NOT NULL, \`id_benefit\` varchar(4) NOT NULL, \`application_date\` date NOT NULL, \`delivery_date\` date NULL, \`status\` enum ('PENDING', 'DELIVERED', 'EXPIRED') NOT NULL DEFAULT 'PENDING', PRIMARY KEY (\`token\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`PartnersAdmins\` (\`id_account\` varchar(4) NOT NULL, \`id_partner\` varchar(4) NOT NULL, PRIMARY KEY (\`id_account\`, \`id_partner\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`PartnersAdmins\` ADD CONSTRAINT \`FK_b43cdde5e6118c5bae9670806a5\` FOREIGN KEY (\`id_account\`) REFERENCES \`Accounts\`(\`id_user\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`PartnersAdmins\` DROP FOREIGN KEY \`FK_b43cdde5e6118c5bae9670806a5\``);
