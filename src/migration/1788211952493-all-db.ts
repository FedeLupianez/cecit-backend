...
export class AllDb1788211952493 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`Users\` (\`id_user\` varchar(4) NOT NULL, \`name\` varchar(50) NOT NULL, \`lastname\` varchar(50) NOT NULL, \`dni\` varchar(8) NOT NULL, INDEX \`IDX_d9de24daa15c9441939de28998\` (\`dni\`), PRIMARY KEY (\`id_user\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`Accounts\` (\`id_user\` varchar(4) NOT NULL, \`email\` varchar(50) NULL, \`password\` varchar(255) NULL, \`role\` enum ('USER', 'CECIT_ADMIN', 'PARTNER_ADMIN') NOT NULL DEFAULT 'USER', \`last_activity\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP(), \`active\` tinyint NOT NULL DEFAULT 1, INDEX \`IDX_0c5666efc38b6f023b7814c73d\` (\`email\`), INDEX \`IDX_0c9042702a3047d28d0a1e68d2\` (\`role\`), UNIQUE INDEX \`IDX_0c5666efc38b6f023b7814c73d\` (\`email\`), PRIMARY KEY (\`id_user\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`RefreshTokens\` (\`id_token\` uuid NOT NULL, \`token_hash\` varchar(255) NOT NULL, \`email\` varchar(50) NOT NULL, \`expires_at\` datetime NOT NULL, \`revoked\` tinyint NOT NULL DEFAULT 0, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id_token\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`Partners\` (\`id_partner\` varchar(4) NOT NULL, \`name\` varchar(50) NOT NULL, \`logo\` varchar(255) NOT NULL, \`id_owner\` varchar(4) NOT NULL, \`active\` tinyint NOT NULL DEFAULT 1, UNIQUE INDEX \`REL_4d9e114868b838c8a507e386ac\` (\`id_owner\`), PRIMARY KEY (\`id_partner\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE \`Partners_Admins\` (\`id_user\` varchar(4) NOT NULL, \`id_partner\` varchar(4) NOT NULL, PRIMARY KEY (\`id_user\`, \`id_partner\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`Directions\` (\`id_partner\` varchar(4) NOT NULL, \`direction\` varchar(150) NOT NULL, PRIMARY KEY (\`id_partner\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`Partners\` ADD \`direction\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`Partners\` DROP COLUMN \`direction\``,
    );
