...
export class AddDefaultValueToBenefitCreationDate1780660787382 implements MigrationInterface {
    name = 'AddDefaultValueToBenefitCreationDate1780660787382'
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`Vouchers\` CHANGE \`status\` \`status\` enum ('PENDING', 'DELIVERED') NOT NULL DEFAULT 'PENDING'`);
        await queryRunner.query(`ALTER TABLE \`Benefits\` CHANGE \`date_entered\` \`date_entered\` date NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`RefreshTokens\` DROP COLUMN \`id_token\``);
